...
public class OrgUserGroupDAO implements IDAO<Org, Boolean, String> {
    @Override
    public SqlDB getDB() {
    ...
    public Boolean deleteByUserId(Integer userId) {
        String sql = String.format("delete from %s where userId = %s", Table.ORG_USER_GROUP, userId);
    ...
    public Boolean deleteByOrgId(Integer orgId) {
        String sql = String.format("delete from %s where organizationId  = %d", Table.ORG_USER_GROUP, orgId);
        PreparedStatement pst;
        try {

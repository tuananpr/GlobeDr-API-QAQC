...
public class NotificationUserDAO implements IDAO<Notification, Boolean, Integer> {
    @Override
    public SqlDB getDB() {
    ...
    public Boolean delete(Integer userId) {
        ...
        cmds.add(String.format("delete from %s where %s", Table.NOTIFICATION_USER, where ));
    ...
    public Boolean delete(Notification notification, FieldType fieldType) {
        ...
        String sql = String.format("delete from %s where %s", Table.NOTIFICATION_USER , where);
        PreparedStatement pst;

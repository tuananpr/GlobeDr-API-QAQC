...
public class PostAccessDAO implements IDAO<Object, Boolean, String> {
    @Override
    public SqlDB getDB() {
    ...
    public Boolean deleteByUserId(Integer userId) {
        ...
        cmds.add(String.format("delete from %s where postId in (Select postId FROM %s WHERE createdById = '%d')", Table.POST_ACCESS, Table.POST, userId));
        PreparedStatement pst;
    ...
    public Boolean deleteByUserId(List<Integer> userIds) {
        ...
        // deletePostMsgDocs
        List<String> cmds = new ArrayList<>();
        cmds.add(String.format("delete from %s where postId in (Select postId FROM %s WHERE createdById IN %s)", Table.POST_ACCESS, Table.POST, users));
        PreparedStatement pst;

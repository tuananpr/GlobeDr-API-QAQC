...
public class NotificationDAO implements IDAO<Notification, Boolean, Integer> {
    @Override
    public SqlDB getDB() {
    ...
    public Boolean delete(Integer userId) {
        ...
        cmds.add(String.format("delete from %s where %s", Table.NOTIFICATION, String.format("senderId = '%d'", userId)));
    ...
    public Boolean delete(Notification notification, FieldType fieldType) {
        String sql = String.format("delete from %s where %s", Table.NOTIFICATION , Common.addingConditionFieldsDB(notification, fieldType));
        PreparedStatement pst;

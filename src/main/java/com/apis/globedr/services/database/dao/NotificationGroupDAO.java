...
public class NotificationGroupDAO implements IDAO<Notification, Boolean, Integer> {
    @Override
    public SqlDB getDB() {
    ...
    public Boolean delete(Integer userId) {
        ...
        cmds.add(String.format("delete from %s where %s", Table.NOTIFICATION_GROUP, where ));
    ...
    public Boolean delete(Notification notification, FieldType fieldType) {
        ...
        String sql = String.format("delete from %s where %s", Table.NOTIFICATION_GROUP , where);
        PreparedStatement pst;
